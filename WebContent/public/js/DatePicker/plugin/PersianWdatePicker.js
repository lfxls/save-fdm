 ...
 * 伊朗历日历控件
 * 注意：
 * 1、需要额外定义全局变量ctxLang，取值为：en_US、zh_CN等。如不定义，则使用默认值zh_CN。
 */
...
Ext.onReady(function() {
	 ...
	 * 国际化部分
	 */
	var $PersianCalenderResourceList = {
		"en_US":{ 
			empty:"clear",
			ok:"OK",
			week_Su:"Sun",
	...
	var $PersianCalenderResource = $PersianCalenderResourceList[(typeof(ctxLang )=="undefined")?"zh_CN":ctxLang];/**meta.jsp中定义*******/;
	/**
	 * 常量
	 */
	 ...
	 * 状态控制
	 */
	var sd_g_object; //触发日历控件的INPUT元素
	var cur_d ; //当前选中的日期
	var callBack; //回调函数
	var options = {
		dateFmt:'yyyy-MM-dd',
		needTime:false //不可配置参数，根据dateFmt的值来确定
	};
	var sd_inover=false;
	var sd_afterTime=""; //初始值中的时间字符串
	P$dp = {};
	/**
	 * 入口函数
	 * @param _pi 触发控件的元素
	...
	PdatePicker = function(_pi,_options,_callBack){
		...
		curEvent.cancelBubble=true; //阻止事件传递
		callBack = _callBack;
		...
		var srcEl = _pi;
		/**定位弹出div位置*********/
		var dateEl = document.getElementById("cele_date");
		dateEl.style.display="";
		dateEl.style.zIndex=99;
		var elRound = getAbsoluteLocation(srcEl);
		dateEl.style.left = (elRound.absoluteLeft) + 'px';
		dateEl.style.top = elRound.absoluteTop + elRound.offsetHeight + "px";
		if (elRound.absoluteLeft+dateEl.offsetWidth>document.body.offsetWidth){ //超出页面最右边
			dateEl.style.left = (document.body.offsetWidth-dateEl.offsetWidth) + "px";
		}
		
		/**根据输入框的值，确定日历控件选中哪一天cur_d、和小时的值sd_afterTime*********/
		var oldTime=srcEl.value; //当前选中日期
		var oldTime_date="";
		if (oldTime.length>3){
			oldTime_date=formatPersianDate(parsePersianDate(oldTime,"yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd");
			sd_afterTime=formatPersianDate(parsePersianDate(oldTime,"yyyy-MM-dd HH:mm:ss"), "HH:mm:ss");
		}
		if (oldTime_date.length>3){ //根据输入框的值确定选中哪一天
			var s=oldTime_date.split(sd_separator);
			cur_d={year:parseInt(s[0]),month:(parseInt(s[1],10)-1),day:parseInt(s[2],10)};
		}else{
			cur_d=getPersianDay(now.getFullYear(), now.getMonth(), now.getDate()); //默认选中当天
		}
		/**根据参数，确定日期选择范围（暂不支持）********************/
		var d_start = "";
		var d_end = "";
		if (d_start!=""){
		    if (d_start=="today"){
		    	...
		    	var s=d_start.split(sd_separator);
		...
		if (d_end!=""){
			var s=d_end.split(sd_separator);
		...
		var hourEl = document.getElementById("pdpHour");
		hourEl.value = times.length>0?times[0]:"00";
		var minuteEl = document.getElementById("pdpMinute");
		minuteEl.value = times.length>1?times[1]:"00";
		var secondEl = document.getElementById("pdpSecond");
		secondEl.value = times.length>2?times[2]:"00";
		//根据日期格式来控制时、分、秒输入框的显示
...
//				"onClick=\"event.cancelBubble=true;\" onBlur=\"P$dp.sd_hilayer()\" onMouseout=\"P$dp.sd_lostlayerfocus()\">";
		var i=0
	     ...
	     temp_str+=		"<tr class='MTitle'>" +
	     					"<td width=22px>"+$PersianCalenderResource.week_Su+"</td>" +
	     					"<td width=22px>"+$PersianCalenderResource.week_Mo+"</td>" +
	 						"<td width=22px>"+$PersianCalenderResource.week_Tu+"</td>" +
	 						"<td width=22px>"+$PersianCalenderResource.week_We+"</td>" +
	 						"<td width=22px>"+$PersianCalenderResource.week_Th+"</td>" +
	 						"<td width=22px>"+$PersianCalenderResource.week_Fr+"</td>" +
	 						"<td width=22px>"+$PersianCalenderResource.week_Sa+"</td>" +
	 					"</tr>";
	     for (i=1 ;i<=6 ;i++){
	    	 temp_str+="<tr>";
	    	 for(j=1;j<=7;j++){
	    		 temp_str+="<td id=\"c"+i+"_"+j+"\"id=\"c"+i+"_"+j+"\"  language=\"javascript\" onmouseover=\"P$dp.sd_overcolor(this)\" " +
	     ...
	     temp_str+="<div class='dpControl'>"
	     				+"<div class='clearBtn'><input type='button' tabIndex='6' class='dpButton' onclick=\"P$dp.clear_date()\" onmouseover=\"P$dp.sd_getlayerfocus()\" style='cursor: hand;' value='" + $PersianCalenderResource.empty + "'/></div>"
	     				+"<div class='okBtn'><input type='button' tabIndex='7' class='dpButton' onclick=\"P$dp.ok_date()\" onmouseover=\"P$dp.sd_getlayerfocus()\" style='cursor: hand;' value='" + $PersianCalenderResource.ok + "'/></div>"
	     		  +"</div>";
	     /*********END 底部********/
	     //temp_str+="<iframe src=\"javascript:false\" class=\"WdateDiv\" style=\"position:absolute; visibility:inherit; top:0px; left:0px;z-index:-1; filter='progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)';\"></iframe>";
	     temp_str+="</div>";
	...
	function sd_change_date(temp,mode){
		...
		var minMonth = cele_date_month.options[0].value;
		var maxMonth = cele_date_month.options[cele_date_month.options.length-1].value;
		var minYear = cele_date_year.options[0].value;
		var maxYear = cele_date_year.options[cele_date_year.options.length-1].value;
	    if (mode>0){ //月操作
	    	var t_month;
	        if(mode==1){ //箭头
	        ...
	        if (t_month<minMonth) { //月份小于最小值
	        	cele_date_month.value=maxMonth; //月份设置为最大
	        	sd_change_date(-1,-1); //年-1
	        }else if (t_month>maxMonth){ //月份大于最大值
	        	cele_date_month.value=cele_date_month.options[0].text; //月份设置为最小
	        	sd_change_date(1,-1);//年+1
	        }else{
	        	cele_date_month.value=t_month;
	        	sd_set_cele_date(cele_date_year.value,cele_date_month.value); //生成日选择Table
	        }
	    }else{ //年操作
	    	var t_year;
	    	if(mode==0){ //下拉框
	        ...
	        if (t_year<minYear) { //年超过最小值，则选中最小年的1月
	            cele_date_year.value=minYear;
	            cele_date_month.value=minMonth;
	        }else if (t_year>maxYear){//年超过最大值，则选中最大年的12月
	        	cele_date_year.value=maxYear;
	        	cele_date_month.value=maxMonth;
	        }else{
	...
	P$dp.sd_change_date = sd_change_date;
	/**
	 * 根据年、月生成日期Table数据
	 */
...
//		event.cancelBubble=true;  //阻止事件上传
	...
	function sd_td_click(t_object){
		...
		if(t_object==null){
			...
			day = t_object.innerHTML;
		}
		if (parseInt(day,10)>=1 && parseInt(day,10)<=31 ){ 
			var t_d=new Date(year,month-1,day);
			if (t_d<=sd_date_end && t_d>=sd_date_start){
				...
				if(callBack){ //调用回调函数，出入选中的伊朗历日期、公历日期。
					...
					}else if(options.dateFmt && options.dateFmt=="yyyy"){ //如果是年份选择，则生成年份起始日期值（公历）的隐藏输入框
						var startPersianDay = selDate +sd_separator+ "01" +sd_separator+ "01" ;
						var endPersianDay = (parseInt(year,10)+1) +sd_separator+ "01" +sd_separator+ "01";
						callBack(selDate, selGreDate , persianToGregorian(startPersianDay) , persianToGregorian(endPersianDay));
					}else{
	...
	function ok_date() {
		if(selectedDayObj!=null){
			...
			if(options.dateFmt && (options.dateFmt=="yyyy-MM" || options.dateFmt=="yyyy")){
				sd_td_click(null);
			}else{
				document.getElementById("cele_date").style.display="none";
			}
...
function getAbsoluteLocation(element) {
	 ...
	 var n_left = f_getPosition (element, 'Left'),
		n_top  = f_getPosition (element, 'Top');
	 return { absoluteTop: n_top, absoluteLeft: n_left, offsetWidth: offsetWidth, offsetHeight: offsetHeight };
}
