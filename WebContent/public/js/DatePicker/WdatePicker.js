 ...
 * 2、需要额外定义全局变量isPersianCalender，如果isPersianCalender="1"，则所有的my97控件的调用都转换变成伊朗历调用。
 * 3、全局变量定义的位置必须在引入WdatePicker.js的前面。
 * ***************************/
/**********************************************************************************
 * 修改记录：
 * 1、my97调用增加一个可选参数noPlugin，如果noPlugin="1"，则强制控件为my97模式（即使系统设置了isPersianCalender="1"）。
 *********************************************************************************/
 (function() {
    var $ = {
        $langList: [{
        ...
        weekMethod: "ISO8601",
        position: {},
        lang: (typeof(ctxLang )=="undefined")?"zh_CN":ctxLang,/**meta.jsp中定义*******/
        skin: "default",
        dateFmt: "yyyy-MM-dd",
    ...
    function I(G, _) {
    	...
    	var noPlugin = G.noPlugin || "0"; //是否支持扩展（默认支持），如果传1则任何情况下都不调用扩展
    	if(isPersian && noPlugin != "1"){ //伊朗历，且支持使用扩展插件，则调用伊朗历控件
			if(!(G.dateFmt=="HH:mm:ss" ||G.dateFmt=="HH:mm" ||G.dateFmt=="mm:ss" 
				||G.dateFmt=="HH" ||G.dateFmt=="mm"||G.dateFmt=="ss")){ //只有时分秒选择，则强制不支持扩展
				PdatePicker(persianEl ,G , function(selDate, selGreDate , monthStartDay , monthEndDay){ //调用伊朗历控件
					var temp_gregorianEl = persianEl.srcEl;
					temp_gregorianEl.value = selGreDate;
					var monthStartDayEl = document.getElementById(temp_gregorianEl.id+"Start");
					var monthEndDayEl = document.getElementById(temp_gregorianEl.id+"End");
		 ...
		 *  公历月份选择，则特殊处理
		 ***********************/
		G.onpicked = function(dp){
			if(isPersian){ //伊朗历模式，某些情况下依然使用my97控件，此时则需要将控件获取的值赋给隐藏的公历输入框
				var gregorianEl = this.srcEl;
				gregorianEl.value = this.value;
			}else{ //公历模式的月份选择进行特殊处理
				var monthStartDayEl = document.getElementById(this.id+"Start");
				var monthEndDayEl = document.getElementById(this.id+"End");
				if(monthStartDayEl && monthEndDayEl && dp.dateFmt && dp.dateFmt=="yyyy-MM"){
					var sd_separator = "-";
